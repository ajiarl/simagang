            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    @foreach($applications as $application)
                        @php
                            $assessment = $application->assessments->where('assessor_type', 'dosen')->first();
                        @endphp
    ...
    @if(!$assessment)
        ...
        <div id="modal-{{ $application->id }}" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 50; align-items: center; justify-content: center;">
            <div class="card" style="width: 100%; max-width: 600px; max-height: 90vh; overflow-y: auto; background: #fff; border-radius: 12px; margin: 20px;">
                ...
                <form action="{{ route('dosen.assessments.store', $application) }}" method="POST">
                    @csrf
                    <div style="padding: 20px; display: flex; flex-direction: column; gap: 16px;">
                        <p class="text-body-sm" style="color: #424751;">Berikan nilai 0 hingga 100 untuk masing-masing aspek berikut:</p>
                    ...
                    <div style="padding: 16px 20px; border-top: 1px solid #c2c6d3; display: flex; justify-content: flex-end; gap: 12px; background: #f8fafc; border-radius: 0 0 12px 12px;">
